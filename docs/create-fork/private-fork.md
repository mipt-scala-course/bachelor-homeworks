...
3. Каждое задание лежит в своей ветке. 
   1. Перед выполнением задания нужно забрать все ветки из upstream (команда `git fetch upstream` или из меню IDEA)
   2. Перейти в ветку задания и забрать ее из upstream (команда `git pull` или из меню IDEA)
   3. Запушить ветку в свой репозиторий на github (команда `git push origin`)
   > Если этого не сделать, то не получится сделать Pull Request с результатом задания!
   4. Создать от ветки задания локальную ветку, где написать решение
   5. Запушить эту ветку в свой репозиторий на github и создать Pull Request в ветку задания
